...
def train(param, num_round=1000, early_stopping_rounds=20):
    ...
    json.dump(train_params, open('{0}_{1}{2}'.format(model_path, exec_time, model_params), 'wb+'))#将训练参数写入文件

    print('get training data')

    train_features = pd.read_csv(train_feature_save_path + 'train_features.csv').astype(float)
    train_labels = pd.read_csv(train_feature_save_path + 'labels.csv').astype(float)

    validate_features = pd.read_csv(validate_feature_save_path + 'train_features.csv').astype(float)
    validate_labels = pd.read_csv(validate_feature_save_path + 'labels.csv').astype(float)

    predict_features = pd.read_csv(predict_feature_save_path + 'train_features.csv').astype(float)

    create_feature_map(train_features.columns.tolist(), '{0}_{1}{2}'.format(model_path, exec_time, model_fmap_file))#存储训练特征列名称

    #xgboost 自带分布式的数据集定义格式，相当有效地处理大型数据集
    ...
    train_check_matrix = train_coupons[['Coupon_id']].join(train_true_frame).join(train_pred_frame)
    val_check_matrix = val_coupons[['Coupon_id']].join(val_true_frame).join(val_pred_frame)
    ...
    submission = submission[['User_id', 'Coupon_id', 'Date_received']].join(frame)
